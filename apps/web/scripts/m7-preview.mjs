...
Object.assign(env, { DATABASE_SSL: "false", SECURITY_DATA_KEY: Buffer.alloc(32, 53).toString("base64"), NEXTAUTH_SECRET: "m7-local-only-synthetic-session-secret", NEXTAUTH_URL: "http://localhost:3005", NEXTAUTH_URL_INTERNAL: "http://localhost:3005", APP_BASE_URL: "http://localhost:3005", TICKETCHILE_QR_SECRET: "m7-local-only-synthetic-qr-secret", STRIPE_SECRET_KEY: "sk_test_disabled_placeholder", RESEND_API_KEY: "re_disabled_placeholder" });
const crypto = loadSource("lib/security/crypto.server.ts");
const user = randomUUID();
await db.pool.query("INSERT INTO usuarios(id,nombre,email,password_hash,email_verified_at) VALUES($1,'Persona de prueba','buyer@m7.test',$2,now())", [user, await crypto.hashPassword("M7-local-fixture-password!")]);
await db.pool.query("INSERT INTO organizer_users(id,username,display_name,password_hash,verified,approved) VALUES('m7-organizer','m7-organizer','Producciones de prueba','disabled',true,true)");
...
for (let i = 0; i < names.length; i++) {
  const id = `m7-event-${i}`, assets = ["fiesta-verano", "sunset-party", "noche-rock"][i % 3];
  ...
  await db.pool.query("INSERT INTO organizer_events(event_id,organizer_id) VALUES($1,'m7-organizer')", [id]);
...
await db.pool.query("INSERT INTO holds(id,event_id,status,expires_at,owner_email) VALUES('m7-hold','m7-event-0','CONSUMED',now(),'buyer@m7.test')");
await db.pool.query("INSERT INTO orders(id,hold_id,event_id,event_title,buyer_name,buyer_email,owner_email) VALUES('m7-order','m7-hold','m7-event-0','Ritual Nocturno','Persona de prueba','buyer@m7.test','buyer@m7.test')");
await db.pool.query("INSERT INTO tickets(id,order_id,event_id,ticket_type_id,ticket_type_name,buyer_email,owner_email,status) VALUES('m7-ticket','m7-order','m7-event-0','general','General','buyer@m7.test','buyer@m7.test','VALID')");

process.env.SECURITY_DATA_KEY=env.SECURITY_DATA_KEY;
env.AI_PROVIDER='development';env.AI_PUBLIC_HOURLY_LIMIT='20';env.AI_USER_HOURLY_LIMIT='100';env.AI_GLOBAL_HOURLY_LIMIT='1000';
const identity=loadSource('lib/security/identity.server.ts',{'@/lib/db':db});
const owner=await identity.principal('ORGANIZER','m7-organizer');
...
await db.pool.query("UPDATE events SET lifecycle='DRAFT',is_published=false WHERE id='m7-event-2'");
await db.pool.query("UPDATE ticket_types SET sold=1 WHERE event_id='m7-event-0' AND id='general'");
await db.pool.query("INSERT INTO payments(id,hold_id,provider,event_id,event_title,buyer_name,buyer_email,owner_email,amount_clp,currency,status,verified_at,paid_at,order_id,fulfillment_status,creation_state) VALUES('m7-payment','m7-hold','stripe','m7-event-0','Ritual Nocturno','Persona de prueba','buyer@m7.test','buyer@m7.test',12000,'CLP','PAID',now(),now(),'m7-order','ISSUED','READY')");
await db.pool.query("INSERT INTO hold_items(hold_id,event_id,ticket_type_id,ticket_type_name,unit_price_clp,qty) VALUES('m7-hold','m7-event-0','general','General',12000,1)");
await db.pool.query("INSERT INTO organizer_staff(id,organizer_id,buyer_id,role,capabilities,event_ids) VALUES($1,'m7-organizer',$2,'ORGANIZER_DOOR',ARRAY['scanner.read','scanner.checkin'],ARRAY['m7-event-0'])",[randomUUID(),user]);
fs.mkdirSync('.local',{recursive:true});fs.writeFileSync('.local/m7-session.json',JSON.stringify({session,database:db.database}));
await db.pool.end();
fs.mkdirSync(".local", { recursive: true }); fs.writeFileSync(".local/m7-preview.json", JSON.stringify({ database: db.database }));
