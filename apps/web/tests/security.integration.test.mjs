...
test("migrations execute and are idempotent; checksum drift is rejected",async()=>{
  await migrate(db.pool);assert.equal((await db.pool.query("SELECT count(*)::int AS n FROM schema_migrations")).rows[0].n,3);
...
test("migration runner refuses unbaselined existing data and rolls back a failed migration",async()=>{
  ...
  for(const file of ["0001_runtime_baseline.sql","0002_identity_security.sql","0003_capability_policy.sql"])
    await fs.copyFile(new URL(file,source),path.join(directory,file));
  await fs.writeFile(path.join(directory,"0004_failure.sql"),"CREATE TABLE should_rollback(id int); SELECT intentionally_missing_function();");
  ...
  assert.equal((await db.pool.query("SELECT count(*)::int AS n FROM schema_migrations")).rows[0].n,3);
});
