...
export default function CheckoutClient({ eventId }: { eventId: string }) {
  ...
  const regionName = useMemo(() => regions.find((x) => x.codigo === regionCode)?.nombre || "", [regions, regionCode]);
  const comunaName = useMemo(() => comunas.find((x) => x.codigo === comunaCode)?.nombre || "", [comunas, comunaCode]);
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (status === "PAID") {
        // Pago ya estaba resuelto (reintento idempotente)
        if (paymentId) {
