...
export default async function Layout({children}:{children:React.ReactNode}){
 ...
 return <OrganizerShell kind={ctx.actor.kind} names={ctx.organizations.map(o=>o.name).join(' / ')} role={ctx.actor.kind==='ORGANIZER'?'Propietario':ctx.organizations.map(o=>roleNames[o.role]).join(' / ')} canCreate={ctx.organizations.some(o=>o.can_create)}><HandoffNotice/>{children}</OrganizerShell>;
}
