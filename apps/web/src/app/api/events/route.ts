...
export async function GET(req: NextRequest) {
  ...
  const slug = pickString(url.searchParams.get("slug"));
  ...
  // Lookup directo (para tu getEventBySlug/getEventById que prueban /api/events?slug=... o ?id=...)
  if (slug || id) {
    const q = await pool.query(
      `
      SELECT ${selectCols}
      FROM events e
      WHERE ${slug ? "e.slug = $1" : "e.id = $1"}
      LIMIT 1
      `,
      [slug || id]
    );

    if (q.rowCount === 0) return NextResponse.json({ ok: false, error: "not_found" }, { status: 404 });
