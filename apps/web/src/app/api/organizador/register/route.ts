...
export async function POST(req: NextRequest) {
  ...
  const orgType = pickString(body, ["orgType", "type"]); // (por ahora no se guarda)
  ...
  const channelRaw = pickString(body, ["channel", "verificationChannel"]);

  const username = pickString(body, ["username", "user"]).toLowerCase() || email;
  ...
  try {
    ...
    const organizerId = ins.rows?.[0]?.id;
    if (!organizerId) throw new Error("No se pudo crear organizer.");
    ...
    await client.query(
      ...
      `,
      [vid, organizerId, channel, destination, code]
    );
    ...
    if (channel === "email") {
      ...
      await sendOrganizerVerificationEmail({ to: destination, code });
    } else {
      throw new Error("WhatsApp aún no está habilitado.");
    }

    return NextResponse.json({ ok: true, organizerId, channel });
  } catch (e: any) {
    try {
      await client.query("ROLLBACK");
    } catch {}
