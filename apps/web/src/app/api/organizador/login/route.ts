...
export async function POST(req: NextRequest) {
  try {
    ...
    const nextRaw = pickString(body, ["from", "next", "redirect", "redirectTo"]);
    ...
    // ✅ Bloqueos profesionales (Opción B)
    if (!user.verified) {
      if (isProbablyBrowserForm(req)) {
        const url = new URL("/organizador/login?reason=unverified", req.url);
        return NextResponse.redirect(url, { status: 303 });
      }
      return NextResponse.json({ ok: false, error: "Debes verificar tu correo primero." }, { status: 403 });
    }

    if (!user.approved) {
      if (isProbablyBrowserForm(req)) {
        const url = new URL("/organizador/login?reason=pending", req.url);
        return NextResponse.redirect(url, { status: 303 });
      }
      return NextResponse.json({ ok: false, error: "Tu cuenta está pendiente de aprobación." }, { status: 403 });
    }
    ...
    return NextResponse.json({ ok: false, error: e?.message || "Error interno en login organizador." }, { status: 500 });
  }
}