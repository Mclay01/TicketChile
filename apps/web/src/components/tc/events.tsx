...
export function EventMetadata({ event }: { event: Event }) {
  return <dl className="metadata"><div><dt>Fecha y hora</dt><dd className="mono">{event.dateISO ? dateLabel(event.dateISO, true,event.timezone) : 'Fecha por confirmar'}</dd>{event.endISO&&<dd className="hint">Hasta {dateLabel(event.endISO,true,event.timezone)}</dd>}<dd className="hint">{event.timezone||'America/Santiago'}</dd></div><div><dt>Recinto</dt><dd>{event.venue}</dd><dd className="muted">{event.city}</dd>{event.address&&<dd className="hint">{event.address}</dd>}</div></dl>;
}
