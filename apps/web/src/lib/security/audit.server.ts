...
export type AuditEntry = {
  ...
  organizerId?: string; eventId?: string; metadata?: { role?: string; capability?: string; outcome?: string; provider?: string; model?:string; fields?:string };
};
export async function audit(client: Pool | PoolClient, entry: AuditEntry) {
  ...
  const metadata = Object.fromEntries(Object.entries(entry.metadata || {}).filter(([key, value]) =>
    ["role", "capability", "outcome", "provider", "model", "fields"].includes(key) && typeof value === "string" && value.length <= (key==='fields'?500:80)));
