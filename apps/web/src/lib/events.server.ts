...
const selection = `SELECT e.*, c.name AS category_name, org.display_name AS organizer_name,
  ...
  LEFT JOIN LATERAL (SELECT jsonb_agg(jsonb_build_object('id',tt.id,'name',tt.name,'price_clp',tt.price_clp,
   ...
   FROM ticket_types tt WHERE tt.event_id=e.id) t ON true`;
...
export async function catalogDb(input: SearchValues = {}) {
  ...
  const where = `e.is_published AND e.date_iso >= now() AND
  ...
  const sort = filters.sort === "date" ? "e.date_iso ASC" : `(SELECT min(price_clp) FROM ticket_types WHERE event_id=e.id) ${filters.sort === "price_desc" ? "DESC" : "ASC"} NULLS LAST`;
...
export async function discoveryFacets() {
  const [categories, cities] = await Promise.all([
    pool.query<{ slug: string; name: string; count: number }>(`SELECT c.slug,c.name,count(e.id)::int AS count FROM event_categories c
      LEFT JOIN events e ON e.category_slug=c.slug AND e.is_published AND e.date_iso>=now() GROUP BY c.slug ORDER BY c.position,c.slug`),
    pool.query<{ city: string; count: number }>(`SELECT city,count(*)::int AS count FROM events WHERE is_published AND date_iso>=now() GROUP BY city ORDER BY count(*) DESC,city LIMIT 50`),
  ]);
