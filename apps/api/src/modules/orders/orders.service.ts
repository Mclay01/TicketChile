...
export async function createOrder(
  ...
  opts?: { flowToken?: string; flowOrder?: number | null }
) {
  ...
  const order = await ordersRepo.createOrderWithTickets({
    ...
    flowToken: opts?.flowToken ?? null,
    flowOrder: opts?.flowOrder ?? null,
  });

  if (!order) {
    throw new AppError(500, 'No se pudo obtener la orden para enviar el mail');
  }
